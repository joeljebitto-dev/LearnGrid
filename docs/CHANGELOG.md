...
- CHG-018 Implemented `T-011 Dashboards And Portals` with role-aware protected frontend routes, session/profile context, analytics dashboard/report APIs, analytics tables, Selenium smoke scaffolding, tests, and design documentation. `OD-005` was later resolved in `CHG-030`.
...
- CHG-023 Implemented `T-018 Search, Reporting, And Analytics` with analytics-service search index APIs, dashboard aggregate and usage metric management, report generation from `analytics_db`, PostgreSQL search indexing, scoped authorization, tests, and design documentation. `OD-005` was later resolved in `CHG-030`.
- CHG-024 Resolved `OD-001` by selecting Nginx and implemented `T-019 API Gateway` with local TLS termination, service routing, `/api/v1` rewriting, `/api/grades` aliasing, request logging, rate and size limits, CORS handling, gateway smoke tests, and dev-runner integration.
- CHG-025 Implemented `T-020 Kafka Eventing` with the shared `learngrid-events` package, Apache Kafka local infrastructure, base/retry/DLQ topics, Kafka UI, producer and consumer helpers, idempotent analytics/notification/progress handlers, lag commands, tests, and event design documentation.
- CHG-026 Implemented `T-021 Redis Architecture` with the shared `learngrid-redis` package, standardized key naming, Redis-backed auth rate limits/password reset/OTP helpers, hashed course catalog cache keys, course structure locks, analytics dashboard response caching, tests, and Redis design documentation while keeping production Sentinel/Cluster selection open under `OD-003`.
- CHG-027 Implemented `T-022 Security` with shared production security helpers, hardened gateway headers/CORS tests, production secret validation, stronger password/input validation, durable admin account audits, optional content malware scanning, Kubernetes security baseline templates, security tests, and PostgreSQL backup restore verification. `OD-004` was later resolved in `CHG-030`.
- CHG-028 Resolved `OD-003` to on-prem Kubernetes and implemented the `T-023` deployment baseline with backend/frontend/gateway production images, Helm charts, health/metrics endpoints, Grafana-stack observability, GHCR image CI, deployment workflow gates, and static deployment tests. Final CI/CD completion remains pending a real staging deployment smoke run.
- CHG-029 Implemented the `T-024` testing and quality baseline with `drf-spectacular` schema/docs endpoints on every backend service, service schema smoke tests, repo OpenAPI/Kafka contract tests, Compose-backed integration tests, Selenium page-object journey tests, k6 smoke/staging load scripts, root test requirements, and expanded CI quality gates. Final performance and autoscaling verification remains pending a real staging/on-prem run.
