...
urlpatterns = [
    ...
    path(
        "records/<uuid:grade_record_id>/",
        GradeRecordDetailView.as_view(),
        name="grade-record-detail",
    ),
    path(
        "records/<uuid:grade_record_id>/override/",
        GradeOverrideView.as_view(),
        name="grade-record-override",
    ),
    path(
        "records/<uuid:grade_record_id>/publish/",
        GradePublishView.as_view(),
        name="grade-record-publish",
    ),
    path(
        "manual-reviews/<uuid:review_id>/complete/",
        ManualReviewCompleteView.as_view(),
        name="manual-review-complete",
    ),
    path("results/", PublishedResultListView.as_view(), name="published-result-list"),
    path(
        "results/<uuid:result_id>/",
        PublishedResultDetailView.as_view(),
        name="published-result-detail",
    ),
    path(
