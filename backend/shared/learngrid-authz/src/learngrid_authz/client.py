...
def authorization_timeout_seconds(default: float = 2.0) -> float:
    raw_value = getattr(settings, "AUTHORIZATION_CHECK_TIMEOUT_SECONDS", default)
    try:
...
def remote_authorization_check(
    *,
    ...
    auth_service_base_url = (base_url or getattr(settings, "AUTH_SERVICE_BASE_URL", "")).rstrip("/")
    ...
    outbound_request = urlrequest.Request(
        f"{auth_service_base_url}/api/auth/authorization/check/",
    ...
    try:
        with urlrequest.urlopen(
            outbound_request,
            timeout=timeout if timeout is not None else authorization_timeout_seconds(),
        ) as response:
...
def require_remote_permission(
    *,
    token: str | None,
    permission: str | None,
    scope_type: str = "platform",
    scope_id: str | None = None,
    denied_message: str = "You do not have permission to access this resource.",
) -> None:
